...
export const updateCartQuantity = (productId, quantity) => put(`/api/cart/${productId}?quantity=${quantity}`)

/** 删除购物车商品 DELETE /api/cart/{productId} */
export const removeFromCart = (productId) => del(`/api/cart/${productId}`)

/** 选中/取消选中商品 PUT /api/cart/check/{productId} */
export const checkCartItem = (productId, checked) => put(`/api/cart/check/${productId}`, { checked })
